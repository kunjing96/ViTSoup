...
class SuperPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.conv2d(x, self.sampled_weight, self.sampled_bias, stride=self.patch_size, padding=self.proj.padding, dilation=self.proj.dilation) * (self.sampled_scale if self.scale else 1)  # B C Wh Ww
        if self.norm is not None:
            _C, Wh, Ww = x.size(1), x.size(2), x.size(3)
            x = x.flatten(2).transpose(1, 2)
            x = self.norm(x)
            x = x.transpose(1, 2).view(B, _C, Wh, Ww)
    ...
    def flops(self, H, W):
        Ho, Wo = int(np.ceil(H / self.patch_size[0])), int(np.ceil(W / self.patch_size[1]))
        flops = 0
...
class SuperWindowAttention(nn.Module):
    ...
    def set_sample_config(self, sampled_dim=None, sampled_qk_dim=None, sampled_num_heads=None, sampled_proj_drop=None, sampled_attn_drop=None, sampled_window_size=None):
        ...
        if self.scale:
            self.sampled_scale = self.qk_dim / self.sampled_qk_dim
...
class SuperSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=SuperLayerNorm, scale=False):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class SuperBasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., act_layer=nn.GELU, norm_layer=SuperLayerNorm, downsample=None, use_checkpoint=False, scale=False, shift=True):
...
class SuperSwinTransformer(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 act_layer=nn.GELU, norm_layer=SuperLayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, scale=False, shift=True, output_proj=False, **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            if self.output_proj:
                proj = SuperLinear(self.num_features[i_layer], self.num_features[i_layer], scale=scale)
                layer_name = f"proj{i_layer}"
                self.add_module(layer_name, proj)
    ...
    def set_sample_config(self, config):
        ...
        self.sampled_num_features = [int(self.sampled_embed_dim * 2 ** i) for i in range(self.sampled_num_layers)]
        ...
        for i_layer in range(self.sampled_num_layers):
            ...
            if self.output_proj:
                proj_layer = getattr(self, f"proj{i_layer}")
                proj_layer.set_sample_config(sampled_in_features=self.sampled_num_features[i_layer])
                norm_layer = getattr(self, f"norm{i_layer}")
                norm_layer.set_sample_config(sampled_embed_dim=self.num_features[i_layer])
            else:
                norm_layer = getattr(self, f"norm{i_layer}")
                norm_layer.set_sample_config(sampled_embed_dim=self.sampled_num_features[i_layer])
    ...
    def forward_features(self, x):
        ...
        if self.ape:
            ...
            interpolated_sampled_absolute_pos_embed = F.interpolate(
                self.sampled_absolute_pos_embed.transpose(1, 2).view(1, -1, patches_resolution[0], patches_resolution[1]), size=(Wh, Ww), mode="bicubic"
            )
            x = (x + interpolated_sampled_absolute_pos_embed).flatten(2).transpose(1, 2)  # B Wh*Ww C
        else:
            x = x.flatten(2).transpose(1, 2)
        x = self.pos_drop(x)
        ...
        outs = {}
        for i_layer in range(self.sampled_num_layers):
            x_out, H, W, x, Wh, Ww = self.layers[i_layer](x, Wh, Ww)

            if self.output_proj:
                proj_layer = getattr(self, f"proj{i_layer}")
                x_out = proj_layer(x_out)

            norm_layer = getattr(self, f"norm{i_layer}")
            x_out = norm_layer(x_out)

            out = x_out.view(B, H, W, self.sampled_num_features[i_layer]).permute(0, 3, 1, 2).contiguous()
            outs["res{}".format(i_layer + 2)] = out
    ...
    def params(self):
        ...
        for i_layer in range(self.sampled_num_layers):
            ...
            if self.output_proj:
                proj_layer = getattr(self, f"proj{i_layer}")
                params += proj_layer.params()
            norm_layer = getattr(self, f"norm{i_layer}")
            params += norm_layer.params()
        return params
    ...
    def flops(self, H, W):
        ...
        patch_size = to_2tuple(self.patch_size)
        Wh, Ww = H // patch_size[0], W // patch_size[1]
        if self.ape:
            flops += 1 * Wh * Ww * self.sampled_embed_dim
        for i_layer in range(self.sampled_num_layers):
            _H, _W = Wh // (2 ** i_layer), Ww // (2 ** i_layer)
            flops += self.layers[i_layer].flops(_H, _W)
            if self.output_proj:
                proj_layer = getattr(self, f"proj{i_layer}")
                flops += proj_layer.flops(_H * _W)
            norm_layer = getattr(self, f"norm{i_layer}")
            flops += norm_layer.flops(_H * _W)
        return flops


@BACKBONE_REGISTRY.register()
class SuperD2SwinTransformer(SuperSwinTransformer, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        ape = cfg.MODEL.BACKBONE.ABS_POS
        patch_norm = cfg.MODEL.BACKBONE.PATCH_NORM
        use_checkpoint = cfg.MODEL.BACKBONE.USE_CHECKPOINT
        scale=cfg.MODEL.BACKBONE.SCALE
        shift = cfg.MODEL.BACKBONE.SHIFT
        output_proj = not cfg.SEARCH_SPACE.SEM_SEG_HEAD.IS_USED

        super().__init__(img_size=pretrain_img_size,
    ...
    def forward(self, x):
        ...
        assert (
            x.dim() == 4
        ), f"SuperSwinTransformer takes an input of shape (N, C, H, W). Got {x.shape} instead!"
        outputs = {}
        y = super().forward(x)
        for k in y.keys():
