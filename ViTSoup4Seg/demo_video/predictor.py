...
from ..train_net import sample_configs
...
class VisualizationDemo(object):
    ...
    def set_mode(self, mode, sub_config=None, search_space=None):
        assert mode in ['super', 'sub'], 'mode must be set as super or sub'
        self.mode = mode
        if mode == 'sub':
            #assert sub_config is not None, 'sub_config must not be None when mode is sub'
            assert search_space is None, 'search_space must be None when mode is sub'
        else:
            #assert sub_config is None, 'sub_config must be None when mode is super'
            assert search_space is not None, 'search_space must not be None when mode is super'
    ...
    def run_on_video(self, frames):
        ...
        """
        if self.mode == 'super':
            config = sample_configs(self.search_space)
        else:
            config = self.sub_config
        model_module = unwrap_model(self.predictor.model)
